	...
	public void drawLayout(Graphics g) throws MalformedURLException {
		...
		for (int x = 0; x < icon_dim.size(); x++) {
			...
			if (gname.equalsIgnoreCase("WIFI Router"))
				...
				url = new URL("http://117.240.224.20/html/pics/RFID_Antenna.png");
			else
			{
				url = new URL("http://117.240.224.20/html/pics/WIFI_Router.png");
			}
	...
	public static void findFloor() {
		...
		if (floor_id.toString().trim().equalsIgnoreCase("All")) {
			...
			try {
				...
				Connection conn = DriverManager.getConnection("jdbc:postgresql://117.240.224.20:5432/" + dbName, "researcher", "researcher");
