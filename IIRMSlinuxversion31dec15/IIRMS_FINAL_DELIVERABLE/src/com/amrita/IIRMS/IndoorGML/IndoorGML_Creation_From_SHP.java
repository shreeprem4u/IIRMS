...
/*
 * File Name        : IndoorGML_Creation.java
 ...
 * Last Modified    : the 9th of September, 2015
...
public class IndoorGML_Creation_From_SHP {
	 ...
	 */
	public IndoorGML_Creation_From_SHP(String pathValue) {
		try {
	...
	public void XML_Creation() throws Exception {
		try {
			...
			// Adding 'spaceLayer' tag to 'spaceLayerMember' (topographic layer)
			Element spaceLayerTopo = doc.createElementNS(NS, "indoorCore:SpaceLayer");
			spaceLayerTopo.setAttributeNS(gmlNS, "gml:id", "SL1");
			
			// class	--> Element to describe the class type of the layer(topographic,sensor,logical)
			//Adding topographic layer
			Element spaceLayerClass1=doc.createElementNS(NS,"indoorCore:class");
			Text topo_textxml = doc.createTextNode("ClassName");
			topo_textxml = doc.createTextNode("topographic");
			spaceLayerClass1.appendChild(topo_textxml);
			spaceLayerTopo.appendChild(spaceLayerClass1);
			
			//Adding 'spaceLayer' tag to 'spaceLayerMember' (logical layer)
			Element spaceLayerLogical = doc.createElementNS(NS, "indoorCore:SpaceLayer");
			spaceLayerLogical.setAttributeNS(gmlNS, "gml:id", "SL2");
			
			//Adding logical layer
			Element spaceLayerClass2=doc.createElementNS(NS,"indoorCore:class");
			Text logical_textxml=doc.createTextNode("ClassName");
			logical_textxml=doc.createTextNode("logical");
			spaceLayerClass2.appendChild(logical_textxml);
			spaceLayerLogical.appendChild(spaceLayerClass2);
			

			// Adding 'nodes' tag to 'Topographic spaceLayer'
			Element topoNodes = doc.createElementNS(NS, "indoorCore:nodes");
			topoNodes.setAttributeNS(gmlNS, "gml:id", "N1");

			// Adding 'edges' tag to 'Topographic spaceLayer'
			Element topoEdges = doc.createElementNS(NS, "indoorCore:edges");
			topoEdges.setAttributeNS(gmlNS, "gml:id", "E1");
			
			// Adding 'nodes' tag to 'Logical spaceLayer'
			Element logicalNodes = doc.createElementNS(NS, "indoorCore:nodes");
			logicalNodes.setAttributeNS(gmlNS, "gml:id", "N1");

			// Adding 'edges' tag to 'logical spaceLayer'
			Element logicalEdges = doc.createElementNS(NS, "indoorCore:edges");
			logicalEdges.setAttributeNS(gmlNS, "gml:id", "E1");

			for (IndoorGML_Room igmlRoom : IndoorGML_File_Parser.roomList) {
				...
				IndoorGML_File_Parser.rTree.nearestN(ppIn, lipIn, 40, Float.POSITIVE_INFINITY);
				...
				Element rCellspace;
				if (igmlRoom.room_type.trim().toLowerCase().equals("corridor")) {
				...
				for (int i = 0; i < adjacentOnes.size(); i++) {
					...
					topoEdges.appendChild(transitionMember);
				}
				...
				topoNodes.appendChild(stateMember);

				primalSpaceFeaturesTag.appendChild(cellSpaceMember);
			}

			// Repeating the existing code for doorsList
			for (IndoorGML_Doors igmlDoor : IndoorGML_File_Parser.doorsList) {
			...
			for (IndoorGML_Stairs igmlStairs : IndoorGML_File_Parser.stairsList) {
				...
				for(IndoorGML_Stairs stairIn:IndoorGML_File_Parser.stairsList)
				{
			...
			//Start of the constraints(logical) graph
			...
			for (IndoorGML_Room igmlRoom : IndoorGML_File_Parser.roomList) {
				...
				for (IndoorGML_Doors doorObjNow : doorIntersects) {
					...
					Geometry geomIntersection = doorObjNow.door_geom.intersection(igmlRoom.room_geom);
					if (!geomIntersection.isEmpty()) {
						if(doorObjNow.zonalConstraint.toLowerCase().equals("accessible"))
						{
						// Adding the 'connects' link to the corresponding
						// transition.
						Element connects = doc.createElementNS(NS, "connects");
						...
						transition.appendChild(geometryxml);
						transitionMember.appendChild(transition);
						logicalEdges.appendChild(transitionMember);
					}
				}
				}
				...
				state.appendChild(geometryState);
				stateMember.appendChild(state);
				logicalNodes.appendChild(stateMember);
			...
			for (IndoorGML_Doors igmlDoor : IndoorGML_File_Parser.doorsList) {
				...
				for (IndoorGML_Room roomObjNow : roomIntersects2) {
					...
					Geometry geomIntersection = roomObjNow.room_geom.intersection(igmlDoor.door_geom);
					if (!geomIntersection.isEmpty()) {
						if(igmlDoor.zonalConstraint.toLowerCase().equals("accessible"))
						{
						// Adding the 'connects' link to the corresponding
						// transition.
						Element connects = doc.createElementNS(NS, "connects");
			...
			//End of constraints(logical) graph

			spaceLayerTopo.appendChild(topoNodes);
			spaceLayerTopo.appendChild(topoEdges);
			spaceLayerLogical.appendChild(logicalNodes);
			spaceLayerLogical.appendChild(logicalEdges);
			spaceLayerMember.appendChild(spaceLayerTopo);
			spaceLayerMember.appendChild(spaceLayerLogical);
